...
/*   Updated: 2026/02/10 15:59:18 by topiana-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	unsigned int x, unsigned int y, const char *string, const char *font_name, int scale)
{
	...
	while (string[i] != '\0')
	{
		if (string[i] == ' ') yft_draw_space_ascii(img, pixel_put, font, coord, scale);
		else yft_draw_ascii(img, pixel_put, &font->ascii_table[(int)string[i]], coord, scale);
		if (string[i + 1] != '\0')
			yft_draw_spacing(img, pixel_put, font, coord, scale);
